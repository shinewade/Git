...
public class IIL_IIH_FLASH_TEST extends TestMethod {
    ...
    public void setup()
    {
        ...
        {
        activePin_GPIO=new BesDsa_PS1600_Disconnect(ds1).getPinGroup(context, spec_measurement, "FLASH0_SO + FLASH0_SI  + FLASH0_HOLD + FLASH0_WP");
        }
        else if (StaticFields.dev_name == "BGA7273")
        {
        activePin_GPIO=new BesDsa_PS1600_Disconnect(ds1).getPinGroup(context, spec_measurement, "FLASH1_SO + FLASH1_SI  + FLASH1_HOLD + FLASH1_WP");
        }
        //GPIO
        new BesDsa_PMIC(ds1).iMeas_PS1600_DigInOut_Std(activePin_GPIO, "1.8 V", "50 uA", "10 ms", true, "VfIm_H_Parallel");
